...
> = {
  ...
  Logs: {
    title: "Logs",
    tooltip: "View server-side scheduler activity logs.",
    details:
      "Live feed of scheduler worker activity. Shows when batch scrapes run, how many results were collected, and any errors that occurred.",
  },
  "Competitor Battlecards": {
    title: "Competitors",
  ...
  "Website Data": {
    title: "Website Data",
    tooltip: "Manage and edit website content data.",
    details:
...
export function SovereignDashboard({
  ...
  function cycleTheme() {
    ...
    setTheme(next);
    applyTheme(next);
    if (!demoMode) saveSovereignValue(THEME_KEY, next).catch(() => {});
  }
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      // Load workspaces from Neon
      const cloudWorkspaces = await loadSovereignValue<Workspace[]>(
        WORKSPACES_KEY,
        [],
      );
      if (cloudWorkspaces.length > 0) {
        setWorkspaces(cloudWorkspaces);
        // Also load active workspace ID from Neon
        const cloudActiveId = await loadSovereignValue<string>(
          ACTIVE_WS_KEY,
          cloudWorkspaces[0].id,
        );
        setActiveWsId(cloudActiveId);
      } else {
        // First run — create default workspace in Neon
        ...
        saveSovereignValue(WORKSPACES_KEY, [defaultWs]).catch(() => {});
        saveSovereignValue(ACTIVE_WS_KEY, "default").catch(() => {});
      }
    };
    initialize();
  }, [applyTheme]);
  ...
  useEffect(() => {
    ...
    loadSovereignValue<AppState>(key, defaultState).then((data) => {
      if (mounted) {
        ...
        setState(merged);
        // Mark initial load complete so the save effect can fire safely
  ...
  function switchWorkspace(wsId: string) {
    ...
    saveSovereignValue(storageKeyForWorkspace(activeWsId), state);
    setActiveWsId(wsId);
    saveSovereignValue(ACTIVE_WS_KEY, wsId).catch(() => {});
  ...
  function createWorkspace(name: string) {
    ...
    setActiveWsId(ws.id);
    saveSovereignValue(ACTIVE_WS_KEY, ws.id).catch(() => {});
    setShowWsPicker(false);
...
Now analyze all ${competitorList.length} competitors:`,
  ...
  /** Show a loading screen while Neon data is being fetched on mount */
  const loading = !demoMode && !initialLoadDone.current;
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-th-bg text-th-text">
        <div className="flex flex-col items-center gap-4">
          <svg
            className="animate-spin h-10 w-10 text-th-accent"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
          ...
          <p className="text-sm text-th-text-muted">Loading workspace data…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-screen overflow-hidden text-th-text">
      ...
      >
        ...
        <div className="border-t border-th-border px-4 py-2 text-center text-xs leading-relaxed text-th-text-muted">
          ...
          <div className="mt-1">GEO/AEO Tracker v1.2</div>
        </div>
      </aside>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-th-border bg-th-card px-3 py-2 md:gap-3 md:px-5 md:py-2.5">
          ...
          {!demoMode && (
            <button
              ...
              className="rounded-md border border-th-border px-2.5 py-1 text-xs font-medium text-th-text-muted hover:bg-th-card-hover hover:text-th-danger transition-colors"
              title="Sign out"
            >
              Logout
            </button>
        ...
        <main className="flex-1 overflow-y-auto bg-th-bg px-3 py-3 md:px-5 md:py-4">
          ...
          <div className={activeTab === "SRO Analysis" ? "" : "hidden"}>
            <section className="rounded-xl border border-th-border bg-th-card p-5 shadow-sm">
              <SROAnalysisTab selectedModel={state.selectedModel} />
            </section>
          </div>
