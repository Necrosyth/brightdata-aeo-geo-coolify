...
type SROAnalysisTabProps = {
  selectedModel?: string;
};

export function SROAnalysisTab({ selectedModel }: SROAnalysisTabProps) {
    ...
    try {
      ...
      try {
        const resp = await fetch("/api/site-context", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: s.targetUrl, selectedModel }),
        });
        if (resp.ok) {
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
                {s.platforms.map((p) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-lg font-bold ${p.targetUrlCited ? "text-green-400" : "text-th-text-muted"}`}
                    >
                      {p.status === "done"
                        ? p.targetUrlCited
                          ? "✓"
