...
import { getModelConfig, isThinkingModel } from "./models-config";
...
/**
 * Unified LLM helper that routes to the appropriate provider
 * (OpenCode Zen or NVIDIA NIM) based on the selected model.
 */
export async function callLLM(
  ...
  if (!config) {
    throw new Error(
      `Unknown model: ${model}. Check models-config.ts for available models.`,
    );
  }

  if (config.provider === "nvidia") {
    return callNvidiaNim(model, systemPrompt, userPrompt, options);
  } else if (config.provider === "opencode-zen") {
    return callOpenCodeZen(model, systemPrompt, userPrompt, options);
  } else {
...
async function callNvidiaNim(
  ...
  // Add thinking budget for Nemotron models
...
async function callOpenCodeZen(
  ...
  if (!apiKey) {
    throw new Error("OPENCODE_ZEN_API_KEY environment variable is not set.");
  }
  ...
  const response = await fetch(
    "https://api.opencode-zen.com/v1/chat/completions",
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${apiKey}`,
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "Unknown error");
    throw new Error(
      `OpenCode Zen API returned ${response.status}: ${errorText}`,
    );
  }
...
/**
 * Runs the SRO analysis using the selected model.
 * Now accepts an optional `selectedModel` parameter.
 */
export async function analyzeSRO(
  ...
  try {
    const content = await callLLM(SYSTEM_PROMPT, userPrompt, selectedModel, {
      temperature: 0.3,
      maxTokens: 4096,
